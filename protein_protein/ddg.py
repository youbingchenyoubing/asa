...
def caculatemethod1(line1,line,line2,complexDDG,scoreDDG):
    ...
    for i in theline:
        if i=='':
            theline.remove(i)
    dirname+='ore'
    theline1=linecache.getline(dirname,2)
    theline1=theline1.strip('\n')
    thelinesplit1=theline1.split(' ')
    ...
    scoreddg=(float(thelinesplit1[1])-float(scoreDDG))*0.58
    f_file=open(dirname,'r+')
    f_file.read()
    if compareddg<2.0 and compareddg>-2.0 :
        f_file.write(line1+':'+theline[5]+':'+str(compareddg)+':'+'N'+':'+thelinesplit1[1]+':'+str(scoreddg)+'\n')
    else:
        f_file.write(line1+':'+theline[5]+':'+str(compareddg)+':'+'Y'+':'+thelinesplit1[1]+':'+str(scoreddg)+'\n')
    f_file.close()
